...
2. The server sets `restaurants.live_snapshot_id` to that snapshot and revalidates the site's own ISR'd pages (`/`, `/menu`) on-demand — see `revalidatePublicMenuSurfaces()` in `src/lib/publish/service.ts`. Nothing was built or redeployed.
   ...
   > **How the flip is authorized (changed 2026-07-16).** The snapshot insert in step 1 is enforced by RLS on the caller's session client, as always. The flip itself is **not** — it's a service-role write now. `restaurants` UPDATE became owner-only (migration `..033`, docs/03) so an employee can't edit the public profile straight from the database, but publishing *is* an employee's job (docs/04's role gate allows it) and publishing means updating this very row. Postgres RLS is row-level, not column-level, so "employees may write these two columns and no others" cannot be a policy — it has to live in code. It stays scoped: `restaurantId` comes from `getRestaurantIdOrThrow()` on the *session* client (a caller can only target their own restaurant), the snapshot was created under RLS moments earlier, and the route sits behind the middleware's auth + role gate. The scheduled path below has always worked this way.
